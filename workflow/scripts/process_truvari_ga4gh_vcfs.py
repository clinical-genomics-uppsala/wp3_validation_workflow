...
def get_args():
    ...
    parser.add_argument("--output",
                        ...
                        metavar="FILE",
                        default=None,
                        help="Output stats file (default: stdout)")
...
def run_snakemake(snakemake):
    """Entry point when called via Snakemake's script: directive.

    Expected Snakemake rule inputs/outputs:
        input:  base_vcf  – GA4GH truth/base VCF (can be gzipped)
                comp_vcf  – GA4GH query/comp VCF (can be gzipped)
        output: stats     – path for the text summary file
    """
    stats = SizeStratifiedTruthStats([0, 500, 5000])
    process_vcf(snakemake.input.base_vcf, stats, True)
    process_vcf(snakemake.input.comp_vcf, stats, False)

    with open(snakemake.output.stats, "w") as fh:
        stats.report(fh)


if "snakemake" in dir():
    run_snakemake(snakemake) 
else:
    main()
